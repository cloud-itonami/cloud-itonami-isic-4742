(ns avretailops.operation
  "OperationActor -- one coordination request = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor
  (AudioVideoRetailAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the AudioVideoRetailGovernor
  ...
  (:require [langgraph.graph :as g]
            ...
            [avretailops.governor :as governor]
...
(defn build
  ...
  `avretailops.store/Store`).
  opts:
    :advisor      -- an `avretailops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; AudioVideoRetailAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; AudioVideoRetailGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
