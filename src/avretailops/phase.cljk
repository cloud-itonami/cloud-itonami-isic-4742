(ns avretailops.phase
  "Phase 0->3 staged rollout for the ISIC-4742 'Retail sale of audio and
  video equipment in specialized stores' operations-coordination actor.

    Phase 0  read-only              -- no writes, still governor-gated.
    Phase 1  assisted-logging       -- sales-record logging allowed,
                                       every write needs human approval.
    Phase 2  assisted-coordination  -- adds floor-staff/delivery
  ...
  rollout milestone still to come. Flagging a warranty concern always
  needs a human to actually look at it.
  `avretailops.governor`'s own `always-escalate-ops` enforces the same
  invariant independently -- two layers, not one, agree on this."
  (:require [avretailops.governor :as governor]))

(def read-ops #{})
(def write-ops governor/allowed-ops)

;; NOTE the invariant: `:flag-warranty-concern` is a member of
...
(defn verdict->disposition
  "Map an AudioVideoRetailGovernor verdict to a base disposition before
  the phase gate."
  [verdict]
