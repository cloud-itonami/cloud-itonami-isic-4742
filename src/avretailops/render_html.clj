(ns avretailops.render-html
  ...
  (`avretailops.operation` -> `avretailops.governor` ->
  `avretailops.store`) through a scenario adapted from this repo's
  own `avretailops.sim` demo driver (`clojure -M:dev:run`, confirmed
  ...
  `vendor-2` -- ids that DO match `avretailops.store/demo-data`, so it
  was safe to reuse rather than author from scratch), trimmed to a
  representative subset (clean auto-commits, always-escalate ops that a
  human then approves, and five distinct HARD-hold reasons) and rendered
...
(def ^:private coordinator
  {:actor-id "coord-1" :actor-role :av-store-coordinator :phase 3})
...
(defn run-demo!
  ...
  warranty-concern flag (ALWAYS escalates -- approved); store-99
  HARD-holds as unregistered; store-3 HARD-holds as registered-but-
  ...
  proposal that drifts into warranty-claim-finalization scope HARD-holds
  ...
  (let [db (store/seed-db)
    ...
    (exec! actor "t5-flag" {:op :flag-warranty-concern :store-id "store-1"
                            :patch {:concern "customer-reported speaker rattling defect, suspected counterfeit HDMI cables in returned bundle"
                                    :confidence 0.92}})
    ...
    (exec! actor "t8-vendor" {:op :coordinate-supply-order :store-id "store-1"
                              :patch {:item "grey-market headphone import" :quantity 40
...
(def ^:private action-gate-rows
  ...
  ;; (README `Features`, `avretailops.governor`/`avretailops.phase`) --
  ...
  ["        <tr><td><code>:log-sales-record</code></td><td><span class=\"ok\">phase-3 auto-commit when clean &amp; high confidence</span></td></tr>"
   ...
   "        <tr><td><code>:flag-warranty-concern</code></td><td><span class=\"warn\">ALWAYS human approval &middot; never auto at any phase &middot; surfaces concern only, never finalizes a warranty claim</span></td></tr>"])
...
(defn render
  ...
  (let [ledger (vec (store/ledger db))
    ...
    (str
     "<html><head><meta charset=\"utf-8\"><title>cloud-itonami-isic-4742 &middot; specialized-av-retail</title><style>"
     (jp-go-dds.skin/dds+skin)
     "</style></head><body>\n"
     "<header class=\"bar\">\n"
     "  <h1>Retail sale of audio and video equipment in specialized stores (ISIC 4742) — Operator Console</h1>\n"
     ...
     "<main>\n"
     "  <section class=\"card\">\n"
     "    <h2>Specialized AV retail stores</h2>\n"
     "    <p class=\"muted\">Demo snapshot — build-time-generated from <code>avretailops.store</code> via <code>avretailops.render-html</code> (<code>clojure -M:dev:render-html</code>), regenerated from the real actor stack.</p>\n"
     "    <table>\n"
     ...
     "  </section>\n"
     "  <section class=\"card\">\n"
     "    <h2>Supply-order vendors</h2>\n"
     "    <p class=\"muted\">A <code>:coordinate-supply-order</code> must name a registered &amp; verified vendor — grey-market / counterfeit AV import brokers are the failure mode this gate permanently blocks.</p>\n"
     "    <table>\n"
     ...
     "  </section>\n"
     "  <section class=\"card\">\n"
     "    <h2>Action gate (AudioVideoRetailGovernor)</h2>\n"
     "    <p class=\"muted\">HARD holds cannot be overridden: store unverified (business registration + retail license), vendor unverified (for supply orders), effect not <code>:propose</code>, and permanently out-of-scope warranty-claim finalization (approve/deny/payout/refund/replacement). An unregistered or unverified store or vendor never reaches a human.</p>\n"
     "    <table>\n"
