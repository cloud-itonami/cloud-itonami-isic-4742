(ns avretailops.sim
  ...
  the permanently-excluded warranty-claim-finalization scope."
...
(defn -main [& _]
  (let [db (store/seed-db)
        coordinator-phase-1 {:actor-id "coord-1" :actor-role :av-store-coordinator :phase 1}
        coordinator-phase-3 {:actor-id "coord-1" :actor-role :av-store-coordinator :phase 3}
        actor (op/build db)]
    ...
    (let [r (exec-op actor "t1" {:op :log-sales-record :store-id "store-1"
                                  :patch {:units-sold 12 :returns 1 :stock-count-delta -13}} coordinator-phase-1)]
      (println r)
      (println "-- human AV store coordinator approves --")
    ...
    (println (exec-op actor "t2" {:op :log-sales-record :store-id "store-1"
                                  :patch {:units-sold 8 :returns 0 :stock-count-delta -8}} coordinator-phase-3))
    ...
    (let [r (exec-op actor "t5" {:op :coordinate-supply-order :store-id "store-1"
                                 :patch {:item "flagship OLED display units" :quantity 10 :estimated-cost 4200.0
                                         :vendor-id "vendor-1"}} coordinator-phase-3)]
      (println r)
      (println "-- human AV store coordinator reviews & approves --")
    ...
    (let [r (exec-op actor "t6" {:op :flag-warranty-concern :store-id "store-1"
                                 :patch {:concern "customer-reported speaker rattling defect, suspected counterfeit HDMI cables in returned bundle" :confidence 0.92}} coordinator-phase-3)]
      (println r)
      (println "-- human AV store coordinator reviews & approves --")
    ...
    (println "\n== log-sales-record store-1, advisor drifts into warranty-claim-finalization scope -> HARD hold, permanent ==")
